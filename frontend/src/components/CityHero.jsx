...
function Particles({ kind, accent }) {
    ...
    if (kind === "waves") {
        return (
            <svg style={{ position: "absolute", bottom: 0, left: 0, width: "100%", height: "128px", opacity: 0.4 }} viewBox="0 0 1600 100" preserveAspectRatio="none">
                {[0, 1, 2].map((i) => (
                    <motion.path
                        key={i}
...
export default function CityHero({ country, countryFlag, region, name, tagline, scene, image, accentColor, stats = [] }) {
    ...
    const bgParallaxX = useTransform(sx, (v) => v * 0.6);
    ...
    return (
        <section
            ref={containerRef}
            style={{ position: "relative", minHeight: "100vh", width: "100%", overflow: "hidden", backgroundColor: "#061B33" }}
        >
            {/* ── foto HD + Ken Burns + parallax ────── */}
            ...
            >
                ...
                <motion.div
                    ...
                    style={{
                        ...
                        fontSize: "12px", textTransform: "uppercase", letterSpacing: "0.25em",
                        color: accent, textShadow: "0 2px 12px rgba(0,0,0,0.6)",
                        fontFamily: "Inter, sans-serif", fontWeight: 700,
                    }}
                >
                    <span style={{ fontSize: "26px" }}>{countryFlag}</span>
                    <span>{country} · {region}</span>
                </motion.div>

                {/* nome da cidade */}
                <h1 style={{ marginTop: "20px", lineHeight: 0.92, textShadow: "0 6px 40px rgba(0,0,0,0.55)" }}>
                    <motion.span
                        style={{ display: "block", fontSize: "clamp(3.5rem,9vw,9rem)", fontWeight: 700, color: "#fff", fontFamily: "'Playfair Display', serif" }}
                    ...
                    <motion.span
                        style={{ display: "block", fontSize: "clamp(3.5rem,9vw,9rem)", fontWeight: 700, color: accent, fontFamily: "'Playfair Display', serif" }}
                ...
                <motion.p
                    ...
                    style={{
                        ...
                        textShadow: "0 2px 16px rgba(0,0,0,0.6)", lineHeight: 1.5,
                    }}
                >
                    {tagline}
                </motion.p>
                    ...
                    {stats.map((s, i) => (
                        <div key={i} style={{
                            ...
                            <div style={{
                                ...
                                color: "rgba(255,255,255,0.55)", fontFamily: "Inter, sans-serif", fontWeight: 600,
                            }}>
                                {s.label}
                            </div>
                        </div>
            ...
            >
                ...
                <motion.div animate={{ y: [0, 8, 0] }} transition={{ duration: 1.8, repeat: Infinity, ease: "easeInOut" }}>
                    <ChevronDown size={18} style={{ color: "rgba(255,255,255,0.45)" }} />
                </motion.div>
            </motion.div>
        </section>
    );
